...
class User(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    username = Column(String(150), unique=True, index=True, nullable=False)  # email for OAuth users
    hashed_password = Column(String(200), nullable=True)   # nullable: OAuth users have no password
    role = Column(String(20), default="admin", nullable=False)
    is_active = Column(Boolean, default=True)
    created_at = Column(DateTime(timezone=True), default=func.now())

    # Google OAuth fields (NULL for password-only users)
    google_sub = Column(String(100), unique=True, nullable=True, index=True)  # Stable Google user ID
    display_name = Column(String(150), nullable=True)                          # Full name from Google
